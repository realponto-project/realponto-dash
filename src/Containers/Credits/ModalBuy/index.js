...
const ModalBuyCredits = ({
  ...
  const [alxaProductId, setAlxaProductId] = useState()

  useEffect(() => {
    const activateItem = find(propEq('isActivate', true), credtisToBuy)

    setAlxaProductId(activateItem?.id)
  }, [credtisToBuy])
        ...
        {credtisToBuy.map((item, index) => (
          <Col span={8} key={item.id}>
            <CardItem
              {...item}
              isFavorite={index === 0}
              isActivate={alxaProductId === item.id}
              handleClick={setAlxaProductId}
            />
          </Col>
        ))}
        ...
        <Col span={24}>
          <Form
            onFinish={(valuesFormData) =>
              handleSubmit({ ...valuesFormData, alxaProductId })
            }
            layout={'vertical'}
