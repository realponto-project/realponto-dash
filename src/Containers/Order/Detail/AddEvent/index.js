...
const AddEvent = ({
  ...
  if (countTotalStatus && productSelected.status.label === 'pending_analysis') {
    statusForm = statusList.filter(
      (status) =>
        status.label === 'in_analysis' || status.label === 'analysis_return'
    )
  }
  ...
  if (
    countTotalStatus &&
    countTotalStatus.in_analysis === countTotalStatus.pending_analysis
  ) {
    statusForm = statusList.filter(
      (status) => status.label === 'analysis_return'
    )
  }
  ...
  if (productSelected.status.label === 'booking') {
    statusForm = statusList.filter(
      (status) =>
        status.label === 'booking_return' || status.label === 'delivery'
    )
  }
